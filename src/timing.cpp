...
int check_data(timing_data* data, key_data * key){
  ...
  unsigned char key_guesses[KEY_LENGTH][KEY_LENGTH];
  bool accepted_guess[KEY_LENGTH];
      
  int bytes_guessed = 0;
  int prev_bytes_guessed = 0;
      
  for(int i = 0; i < KEY_LENGTH ; i++)
      accepted_guess[i] = false;

  unsigned char * opb = *key;

    int votes[KEY_LENGTH][256];
    memset(votes, 0, sizeof(votes));


    large_table *cost[KEY_LENGTH][KEY_LENGTH];
    for(int i = 0; i < KEY_LENGTH; i++){
	...
	for(int j = i + 1; j < KEY_LENGTH; j++){
	    ...
	    printf("%5d ", count_below((*cost[i][j])[opb[i]][opb[j]], *cost[i][j], 0, 255, 0, 255));
	}
	printf("\n");
    }
	...
	for(int i = 0; i < KEY_LENGTH; i++) for(int j = i + 1; j < KEY_LENGTH; j++){
	    if(!accepted_guess[i] && !accepted_guess[j]){
		double x = find_threshold(cutoff, *cost[i][j]);
    		for(int u = 0; u < 256; u++) 	
		    for(int v = 0; v < 256; v++)
			if((*cost[i][j])[u][v] < x){
			    votes[i][u]++;
	    ...
	    if(!accepted_guess[i] && accepted_guess[j]){
		double x = find_threshold(cutoff, *cost[i][j], 0, 255, key_guess[j], key_guess[j]);
    		for(int u = 0; u < 256; u++) 	
		    if((*cost[i][j])[u][key_guess[j]] < x)
		        votes[i][u] += 2;
	    }

	    if(accepted_guess[i] && !accepted_guess[j]){
		double x = find_threshold(cutoff, *cost[i][j], key_guess[i], key_guess[i], 0, 255);
    		for(int v = 0; v < 256; v++) 	
		    if((*cost[i][j])[key_guess[i]][v] < x)
		        votes[j][v] += 2;
	    }
	}

	int vote_count[KEY_LENGTH]; 
	memset(vote_count, 0, sizeof(vote_count));

	for(int i = 0; i < KEY_LENGTH; i++) 
	    if(!accepted_guess[i]){
		int u = find_max(votes[i], 256);
		vote_count[i] = votes[i][u];
	    }

	int p = find_max(vote_count, KEY_LENGTH);

	if(vote_count[p] > 2){
	    int u = find_max(votes[p], 256);
	    accepted_guess[p] = true;
	    key_guess[p] = u;
	    progress = true;
	    printf("new value k[%2d]=%02x (%d votes)\n", p, u, votes[p][u]);	    
	}
...
void main(int argc, char** argv){
    ...
    key_data REALKEY[16] = {0x0d, 0x54, 0x1d, 0xe3, 0x9e, 0x12, 0x47, 0x72, 0x89, 0x87, 0x95, 0x6c, 0x05, 0xc8, 0x1e, 0x3a};
